...
PSInput.position.x += sin(TIME * 3.5 + 2.0 * l.x + 2.0 * l.z + l.y) * 0.015;
}
#endif

#ifndef BYPASS_PIXEL_SHADER
float4 hsv = rgb2hsv(PSInput.color);
if (hsv.x >= 120.0 && hsv.x <= 270.0) {
float3 l = worldPos.xyz + VIEW_POS;
PSInput.position.y += sin(TIME * 3.5 + 2.0 * l.x + 2.0 * l.z + l.y) * 0.02 * frac(VSInput.position.y);
}
#endif
